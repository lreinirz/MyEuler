...
'''HORIZONTAL MUTIPLYING'''
index0, index1, index2, index3 = 0, 1, 2, 3
horizontal = []
for lists in alphabet:
	index0,	index1,	index2, index3 = 0, 1, 2, 3
	while index3 <20:
		multi = lists[index0]*lists[index1]*lists[index2]*lists[index3]
		horizontal.append(multi)
		index0 += 1
		index1 += 1
		index2 += 1
		index3 += 1
solutions.append(max(horizontal))

'''VERTICAL MULTIPLYING'''
vertical = []
x, y, z = 0, 4, 0
iterations = 0
multiply = 1
...
while x < 17:
	for lists in alphabet[x:y]:
		multiply *= lists[z]
		iterations += 1
		if iterations >= 4:
			iterations = 0
			vertical.append(multiply)
			multiply = 1
			z += 1
			if z >= 20:
				x += 1
				y += 1
				z = 0
solutions.append(max(vertical))

'''DIAGONAL MULTIPLYING'''
x, y = 0, 0
diagonal = []

while x <= 16:
	multiple = alphabet[x][y] * alphabet[x+1][y+1] * alphabet[x+2][y+2] * alphabet[x+3][y+3]
	diagonal.append(multiple)
	y += 1
	if y >= 17:
		x +=1
		y = 0
solutions.append(max(diagonal))

'''REVERSE DIAGONAL MULTIPLYING'''
x, y = 0, 0
diagonal_reverse = []

for reversal_command in alphabet:
	reversal_command.reverse()

while x <= 16:
	multiplication = alphabet[x][y] * alphabet[x+1][y+1] * alphabet[x+2][y+2] * alphabet[x+3][y+3]
	diagonal_reverse.append(multiplication)
	y += 1
	if y >= 17:
		x +=1
		y = 0
solutions.append(max(diagonal_reverse))
print(solutions.index(max(diagonal_reverse)))
print(f"The answer is {max(solutions)}")
