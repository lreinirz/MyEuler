...
def max_path_sum(triangle):
    ...
    for counter in range(-1, -row_count, -1):
        ...
        for each in range(0, len(triangle[counter]) - 1):

            sum1 = triangle[counter][i] + triangle[counter-1][i]
            sum2 = triangle[counter][j] + triangle[counter-1][i]

            triangle[counter-1][i] = max(sum1, sum2)
            i += 1
            j = i + 1        
